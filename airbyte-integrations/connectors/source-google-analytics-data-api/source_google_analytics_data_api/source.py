...
class SourceGoogleAnalyticsDataApi(AbstractSource):
    ...
    def get_property_ids(self, config):
        property_ids = config.get("property_id", [])
        if isinstance(property_ids, str):
            return [property_ids]
        if isinstance(property_ids, list):
            return property_ids
        raise TypeError(f"Unexpected `property_id` type. Expected list, got {type(property_ids)}")
    ...
    def instantiate_report_streams(self, report: dict, config: Mapping[str, Any], **extra_kwargs) -> GoogleAnalyticsDataApiBaseStream:
        for property_id in self.get_property_ids(config):
            _config = config.copy()
            _config["property_id"] = property_id
            yield self.instantiate_report_class(report=report, config=_config)
